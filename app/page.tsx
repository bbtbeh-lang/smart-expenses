...
export default function Home() {
  ...
  const handleApplyCode = (code: string): boolean => {
    // Fire async Supabase check; return true optimistically for local VALID_CODES
    if (VALID_CODES.includes(code)) {
      setState(prev => ({ ...prev, codeActivated: true, scansUsedToday: 0 }));
      addToast(tr.codeSuccess, 'success');
      return true;
    }
    // Check daily_codes table asynchronously
    ...
    // Return false — toast will fire asynchronously
    return false;
  };
  ...
  return (
    ...
    >
      ...
      {/* Floating Action Button — only on dashboard tab */}
